    ...
    if ("error" in res) {
      ...
      if ("data" in res.error) {
        ...
        if (logs && Array.isArray(logs)) {
          const traceIndent = "\n    ";
          traceIndent + logs.join(traceIndent);
        }
      }
...
var ConnectionProvider = ({
  ...
  const connection = createMemo(() => new Connection(endpoint, config), [endpoint, config]);
  return createComponent(ConnectionContext.Provider, {
    get value() {
      return {
...
// src/WalletProviderBase.tsx
function WalletProviderBase(props) {
  ...
  const [publicKey2, setPublicKey] = createSignal(props.adapter?.publicKey ?? null);
  const [connected, setConnected] = createSignal(props.adapter?.connected ?? false);
  function handleErrorRef(error, adapter) {
  ...
  const [wallets2, setWallets] = createSignal(props.wallets.map((adapter) => ({
    adapter,
  ...
  createEffect(() => {
    setWallets((wallets3) => props.wallets.map((adapter, index) => {
      const wallet2 = wallets3[index];
      return wallet2 && wallet2.adapter === adapter && wallet2.readyState === adapter.readyState ? wallet2 : {
        adapter,
        readyState: adapter.readyState
      };
    }).filter(({
    ...
    function handleReadyStateChange(readyState) {
      setWallets((prevWallets) => {
        ...
        }) => readyState2 !== WalletReadyState.Unsupported);
      });
    }
    props.wallets.forEach((adapter) => adapter.on("readyStateChange", handleReadyStateChange, adapter));
    return () => {
      props.wallets.forEach((adapter) => adapter.off("readyStateChange", handleReadyStateChange, adapter));
    };
  });
  const wallet = createMemo(() => wallets2().find((wallet2) => wallet2.adapter === props.adapter) ?? null);
  createEffect(() => {
    if (!props.adapter)
      return;
    ...
    props.adapter.on("connect", handleConnect2);
    props.adapter.on("disconnect", handleDisconnect2);
    props.adapter.on("error", handleError2);
    return () => {
      props.adapter?.off("connect", handleConnect2);
      props.adapter?.off("disconnect", handleDisconnect2);
      props.adapter?.off("error", handleError2);
      handleDisconnect2();
    };
  });
  const [didAttemptAutoConnectRef, setDidAttemptAutoConnectRef] = createSignal(false);
  createEffect(() => {
    return () => {
      if (props.adapter) {
  ...
  createEffect(() => {
    if (didAttemptAutoConnectRef() || isConnectingRef() || connected() || !props.onAutoConnectRequest || !(wallet()?.readyState === WalletReadyState.Installed || wallet()?.readyState === WalletReadyState.Loadable))
      return;
  ...
  async function sendTransaction(transaction, connection, options) {
    if (!props.adapter)
      throw handleErrorRef(new WalletNotSelectedError());
    if (!connected())
      throw handleErrorRef(new WalletNotConnectedError(), props.adapter);
    return await props.adapter.sendTransaction(transaction, connection, options);
  }
    ...
    try {
      ...
      try {
        ...
        if (feature === SolanaSignAndSendTransaction) {
          const [output2] = await __classPrivateFieldGet2(this, _StandardWalletAdapter_wallet, "f").features[SolanaSignAndSendTransaction].signAndSendTransaction({
            account,
            chain,
          ...
          return import_bs582.default.encode(output2.signature);
        } else {
          const [output2] = await __classPrivateFieldGet2(this, _StandardWalletAdapter_wallet, "f").features[SolanaSignTransaction].signTransaction({
            account,
            chain,
            transaction: serializedTransaction,
            options: {
...
_StandardWalletAdapter_account = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_publicKey = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_connecting = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_disconnecting = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_off = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_supportedTransactionVersions = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_wallet = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_readyState = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_changed = /* @__PURE__ */ new WeakMap(), _StandardWalletAdapter_instances = /* @__PURE__ */ new WeakSet(), _StandardWalletAdapter_connect = async function _StandardWalletAdapter_connect2(input) {
  ...
  __classPrivateFieldSet2(this, _StandardWalletAdapter_publicKey, publicKey2, "f");
  __classPrivateFieldGet2(this, _StandardWalletAdapter_instances, "m", _StandardWalletAdapter_reset).call(this);
  ...
  try {
    ...
    let output2;
    try {
      [output2] = await __classPrivateFieldGet2(this, _StandardWalletAdapter_wallet, "f").features[SolanaSignIn].signIn(input);
    } catch (error) {
      throw new WalletSignInError(error?.message, error);
    }
    if (!output2)
      throw new WalletSignInError();
    __classPrivateFieldGet2(this, _StandardWalletAdapter_instances, "m", _StandardWalletAdapter_connected).call(this, output2.account);
    return output2;
  } catch (error) {
    this.emit("error", error);
    throw error;
  ...
  createEffect(() => {
    const listeners2 = [
      on2("register", (...wallets2) => {
        setStandardAdapters((standardAdapters2) => [
          ...standardAdapters2,
      ...
      on2(
        "unregister",
        (...wallets2) => setStandardAdapters(
          (standardAdapters2) => standardAdapters2.filter(
            (standardAdapter) => wallets2.some((wallet) => wallet === standardAdapter.wallet)
          )
        )
      )
    ];
...
var DEFAULT_LOCAL_STORAGE_KEY = "solana-wallet-adapter";
function WalletProvider(props) {
  ...
  const [walletName, setWalletName] = useLocalStorage(props.localStorageKey || DEFAULT_LOCAL_STORAGE_KEY, null);
  const adapter = createMemo(() => adaptersWithStandardAdapters.find((a) => a.name === walletName()) ?? void 0);
  function changeWallet(nextWalletName) {
  ...
  const [hasUserSelectedAWallet, setHasUserSelectedAWallet] = createSignal(false);
  const handleAutoConnectRequest = createMemo(() => {
    if (!props.autoConnect || !adapter())
      return;
    return async () => {
      if (props.autoConnect === true || typeof props.autoConnect !== "boolean" && props.autoConnect && await props.autoConnect(adapter())) {
  ...
  function selectWallet(walletName2) {
    setHasUserSelectedAWallet(true);
    changeWallet(walletName2);
  }
  return createComponent(WalletProviderBase, {
    get wallets() {
      return adaptersWithMobileWalletAdapter();
    },
    get adapter() {
      return adapter();
    },
    get isUnloadingRef() {
      return isUnloadingRef();
    },
    get onAutoConnectRequest() {
      return handleAutoConnectRequest();
    },
...
export { ConnectionContext, ConnectionProvider, WalletContext, WalletNotSelectedError, WalletProvider, useConnection, useWallet };
