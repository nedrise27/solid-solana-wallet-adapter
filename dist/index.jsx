...
var ConnectionProvider = ({
  ...
  return <ConnectionContext.Provider value={{ connection: connection() }}>{children2}</ConnectionContext.Provider>;
};
...
function WalletProviderBase(props) {
  ...
  const [publicKey2, setPublicKey] = createSignal(
  ...
  const [wallets2, setWallets] = createSignal(
    props.wallets.map((adapter) => ({
      adapter,
  ...
  createEffect(() => {
    setWallets(
      (wallets3) => props.wallets.map((adapter, index) => {
    ...
    function handleReadyStateChange(readyState) {
      setWallets((prevWallets) => {
        const index = prevWallets.findIndex(({ adapter: adapter2 }) => adapter2 === this);
        if (index === -1)
          return prevWallets;
        ...
        return [
          ...
          ({ readyState: readyState2 }) => readyState2 !== WalletReadyState.Unsupported
        );
      });
    }
    props.wallets.forEach(
  ...
  const wallet = createMemo(
    () => wallets2().find((wallet2) => wallet2.adapter === props.adapter) ?? null
  );
  createEffect(() => {
    if (!props.adapter)
      return;
...
function WalletProvider(props) {
  ...
  const adapter = createMemo(
    () => adaptersWithStandardAdapters.find((a) => a.name === walletName()) ?? void 0
  );
  ...
  >{props.children}</WalletProviderBase>;
}
export {
  ConnectionContext,
  ConnectionProvider,
